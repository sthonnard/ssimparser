testthat::test_that("CDG is LFPG", {
  testthat::expect_equal((ssimparser::load_ssim() %>% filter(adep_iata == "CDG") %>% select(adep_icao) %>% unique())[[1]], "LFPG")
  })
testthat::test_that("ALC is LEAL", {
  testthat::expect_equal((ssimparser::load_ssim() %>% filter(ades_iata == "ALC") %>% select(ades_icao) %>% unique())[[1]], "LEAL")
})
...
testthat::test_that("Check all columns",
                    {
                      schedules <- ssimparser::load_ssim(ssimparser::get_ssim_sample(), collist = ssimparser::get_ssim_collist(getall = TRUE), nested_df = FALSE)
                      testthat::expect_known_value(data.frame(schedules), "schedules")
                    })

testthat::test_that("Check all columns nested",
                    {
                      schedules <- ssimparser::load_ssim(ssimparser::get_ssim_sample(), collist = ssimparser::get_ssim_collist(getall = TRUE), nested_df = TRUE)
...
"))
  ...
  py$slots_df %>% dplyr::filter(ad == "D" & record_type == 3) %>%
    mutate(passenger_std = stringr::str_sub(scheduled_time_of_passenger_departure, 1, 4),
    ...
    select(iata_airline = airline_designator, flight_number, service_type,
  ...
  ssim <- ssimparser::load_ssim(paste0(tempdir(),"/","ssim.txt"), expand_sched = FALSE, collist = ssimparser::get_ssim_collist(TRUE)) %>% select(all_of(col_to_check))
  # Compare
  print(ssim)
  print(pyssim)
